...
syntax keyword incarKeyword nextgroup=incarValidStatement
    ...
    \ BPARAM BSEELECTRON BSEHOLE BSEPREC CH_LSPEC CH_NEDOS CH_SIGMA CLL CLN CLNT
    \ CLZ CMBJ CMBJA CMBJB CMBJE CPARAM CSHIFT CUTOFF_MU CUTOFF_SIGMA
    \ CUTOFF_TYPE DEPER DIMER_DIST DIPOL DQ EBREAK EDIFF EDIFFG EFERMI EFIELD
    \ EFIELD_PEAD EFOR EINT EMAX EMIN ENAUG ENCUT ENCUTFOCK ENCUTGW ENCUTGWSOFT
    \ ENINI EPSILON ESTOP EVENONLY EVENONLYGW FERDO FERWE FINDIFF FMP_ACTIVE
    \ FMP_DIRECTION FMP_PERIOD FMP_SNUMBER FMP_SWAPNUM FOCKCORR GAMMA_VDW GGA
    \ GGA_COMPAT HFALPHA HFLMAX HFRCUT HFSCREEN HILLS_BIN HILLS_H HILLS_W
    \ HITOLER I_CONSTRAINED_M IALGO IALL_IN_ONE IBAND IBRION ICHARG ICHIBARE
    \ ICORELEVEL IDIPOL IEPSILON IGPAR IMAGES IMIX INCREM INIMIX INIWAV IPEAD
    \ IRC_DELTA0 IRC_DIRECTION IRC_MAXSTEP IRC_MINSTEP IRC_STOP IRC_VNORM0 ISIF
    \ ISMEAR ISPIN ISTART ISYM IVDW IVDW_NL IWAVPR KBLOCK
    \ KERNEL_TRUNCATION/IDIMENSIONALITY KERNEL_TRUNCATION/ISURFACE
    \ KERNEL_TRUNCATION/LCOARSEN KERNEL_TRUNCATION/LTRUNCATE KGAMMA KPAR
    \ KPOINT_BSE KPOINTS_OPT_NKBATCH KPUSE KSPACING LADDER LAECHG LALL_IN_ONE
    \ LAMBDA LANCZOSTHR LANGEVIN_GAMMA LANGEVIN_GAMMA_L LASPH LASYNC
    \ LATTICE_CONSTRAINTS LBERRY LBLUEOUT LBONE LCALCEPS LCALCPOL LCHARG
    \ LCHARGH5 LCHIMAG LCORR LDAU LDAUJ LDAUL LDAUPRINT LDAUTYPE LDAUU LDIAG
    \ LDIPOL LDISENTANGLED LEFG LELF LEPSILON LFINITE_TEMPERATURE LFOCKACE
    \ LFOCKAEDFT LFXC LH5 LHARTREE LHFCALC LHYPERFINE LIBMBD_ALPHA LIBMBD_C6AU
    \ LIBMBD_K_GRID LIBMBD_K_GRID_SHIFT LIBMBD_MBD_A LIBMBD_MBD_BETA
    \ LIBMBD_METHOD LIBMBD_N_OMEGA_GRID LIBMBD_PARALLEL_MODE LIBMBD_R0AU
    \ LIBMBD_TS_D LIBMBD_TS_SR LIBMBD_VDW_PARAMS_KIND LIBMBD_XC LIBXC1 LIBXC1_Pn
    \ LIBXC2 LIBXC2_Pn LKPOINTS_OPT LKPOINTS_WAN LKPROJ LLRAUG LMAXFOCK LMAXMIX
    \ LMAXPAW LMAXTAU LMIXTAU LMODELHF LMONO LMP2LT LNABLA LNMR_SYM_RED
    ...
    \ LSCK Construction:LSCRPA LSCSGRAD LSELFENERGY LSEPB LSEPK LSFBXC LSINGLES
    \ LSMP2LT LSORBIT LSPECTRAL LSPECTRALGW LSPIN_VDW LSPIRAL LSUBROT LSYNCH5
    \ LTBOUNDLIBXC LTEMPER LTHOMAS LTRIPLET LTSSURF LUSE_VDW LUSENCCL LVACPOTAV
    \ LVDW_EWALD LVDW_ONECELL LVDWEXPANSION LVDWSCS LVGVAPPL LVGVCALC LVHAR
